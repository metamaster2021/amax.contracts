...
namespace eosio {
   ...
   /**
    * The `amax.msig` system contract allows for creation of proposed transactions which require authorization from a list of accounts, approval of the proposed transactions by those accounts required to approve it, and finally, it also allows the execution of the approved transactions on the blockchain.
    *
    ...
    * - then you submit this proposal to the `amax.msig` contract, and you also insert the account permissions required to approve this proposal into the command that submits the proposal to the blockchain,
    * - the proposal then gets stored on the blockchain by the `amax.msig` contract, and is accessible for review and approval to those accounts required to approve it,
    * - after each of the appointed accounts required to approve the proposed transactions reviews and approves it, you can execute the proposed transaction. The `amax.msig` contract will execute it automatically, but not before validating that the transaction has not expired, it is not cancelled, and it has been signed by all the permissions in the initial proposal's required permission list.
    */
   class [[eosio::contract("amax.msig")]] multisig : public contract {
      public:
   ...
   struct [[eosio::table, eosio::contract("amax.msig")]] proposal {
      name                                                            proposal_name;
   ...
   struct [[eosio::table, eosio::contract("amax.msig")]] old_approvals_info {
      name                            proposal_name;
   ...
   struct [[eosio::table, eosio::contract("amax.msig")]] approvals_info {
      uint8_t                 version = 1;
      name                    proposal_name;
   ...
   struct [[eosio::table, eosio::contract("amax.msig")]] invalidation {
         name         account;
