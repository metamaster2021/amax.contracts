...
public:
   ...
   eosio_wrap_tester() {
      create_accounts( { N(amax.msig), N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(amax.wrap), contracts::wrap_wasm(), &system_private_key );
      set_abi( N(amax.wrap), contracts::wrap_abi().data(), &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(amax.wrap) );
      abi_def abi;
   ...
   void propose( name proposer, name proposal_name, vector<permission_level> requested_permissions, const transaction& trx ) {
      push_action( N(amax.msig), N(propose), proposer, mvo()
                     ("proposer",      proposer)
   ...
   void approve( name proposer, name proposal_name, name approver ) {
      push_action( N(amax.msig), N(approve), approver, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
   ...
   void unapprove( name proposer, name proposal_name, name unapprover ) {
      push_action( N(amax.msig), N(unapprove), unapprover, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
...
BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, eosio_wrap_tester ) try {
...

   {
      ...
      wrap_trx.actions.emplace_back( get_action( N(amax.wrap), N(exec),
                                                 {{N(alice), config::active_name}, {N(amax.wrap), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, eosio_wrap_tester ) try {
   ...
   push_action( N(amax.msig), N(exec), N(alice), mvo()
                  ("proposer",      "carol")
                  ("proposal_name", "first")
                  ("executer",      "alice")
   );
   ...
   BOOST_REQUIRE_EQUAL( N(amax.wrap), name{traces[0]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_unapprove, eosio_wrap_tester ) try {
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of amax.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(amax.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_producers_change, eosio_wrap_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of amax.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(amax.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
