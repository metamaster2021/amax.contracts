...
public:
   eosio_msig_tester() {
      create_accounts( { N(amax.msig), N(amax.stake), N(amax.ram), N(amax.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(amax.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(amax.msig), name, accounts, data );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   // required to set up the link between (eosio active) and (amax.prods active)
   //
   //                  eosio active
   //                       |
   //             amax.prods active (2/3 threshold)
   ...
   set_authority(
      ...
      authority( 1,
                 ...
                 vector<permission_level_weight>{{{N(amax.prods), config::active_name}, 1}}
      ),
      config::owner_name,
   ...
   create_accounts( { N(amax.token), N(amax.rex) } );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, eosio_msig_tester ) try {
   ...
   // set up the link between (eosio active) and (amax.prods active)
   set_authority(
      config::system_account_name,
      config::active_name,
      authority( 1,
