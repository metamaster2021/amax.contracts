...
static auto proxy_contract_name  =  N(realme.owner);
static auto recover_contract_name  =  N(realme.dao);
...
//    // }
   ...
   action_result recover_action_init( uint8_t recover_threshold, 
       ...
       return push_action(  recover_contract_name, recover_abi_ser, recover_contract_name, N(init), mvo()
           ...
           ( "realme_owner_contract",       realme_owner_contract)
      );
   }
   ...
   action_result proxy_action_init( name realme_dao ) {
      std::cout << "proxy_action_init :" << proxy_contract_name << "," << realme_dao <<"---- end\n"; 

       return push_action(  proxy_contract_name, proxy_abi_ser, proxy_contract_name, N(init), mvo()
            ( "realme_dao",              realme_dao)
      );   
   }
   ...
   action_result auth_action_init( name check_contract, name realme_dao) {
      std::cout << "auth_action_init :" << check_contract << "," << realme_dao <<"---- end\n"; 
